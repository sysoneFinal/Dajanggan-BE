...
public class VacuumAgg5mAggregator {
    ...
    WITH agg_5m AS (
        SELECT 
            ...
            COALESCE(AVG(total_vacuum_sessions), 0)::INTEGER as total_vacuum_sessions,
            COALESCE(AVG(active_vacuum_sessions), 0)::INTEGER as active_vacuum_sessions,
            COALESCE(AVG(autovacuum_sessions), 0)::INTEGER as autovacuum_sessions,
            COALESCE(AVG(manual_vacuum_sessions), 0)::INTEGER as manual_vacuum_sessions,
            COALESCE(AVG(avg_dead_tuples), 0)::BIGINT as avg_dead_tuples,
            MAX(max_dead_tuples) as max_dead_tuples,
            SUM(total_dead_tuples) as total_dead_tuples,
            AVG(avg_progress) as avg_progress,
            COALESCE(AVG(tables_with_dead_tuples), 0)::INTEGER as tables_with_dead_tuples,
