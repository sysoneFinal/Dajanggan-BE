...
/**
 * Disk I/O 5분 집계 스케줄러
 * 5분마다 실행하여 disk_io_agg 데이터를 5분 단위로 재집계
 */
@Slf4j
@Component
@RequiredArgsConstructor
public class DiskIoAgg5mScheduler {
    ...
    @PostConstruct
    public void init() {
        log.info("========== DiskIoAgg5mScheduler 초기화 완료 ==========");
    }
        ...
        try {
            ...
            for (Long instanceId : instanceIds) {
                try {
                    ...
                    failCount++;
                    log.error("Disk I/O 5분 집계 실패: instanceId={}", instanceId, e);
                }
            }

            log.info("========== Disk I/O 5분 집계 완료: 성공={}, 실패={} ==========", 
                    successCount, failCount);

        } catch (Exception e) {
            log.error("Disk I/O 5분 집계 중 오류 발생", e);
        }
    }

    /**
     * 인스턴스별 5분 집계 처리 (backend_type별로)
     */
    private void aggregateInstance5m(Long instanceId, OffsetDateTime timeBucket,
        ...
        // disk_io_agg 테이블에서 5분간 데이터를 backend_type별로 집계
        String sql = """
            SELECT
                ? as time_bucket,
                ? as instance_id,
        ...
        for (Map<String, Object> aggData : results) {
            DiskIoAgg5m agg5m = DiskIoAgg5m.builder()
            ...
            log.debug("Disk I/O 5분 집계 완료: instanceId={}, timeBucket={}, backendType={}", 
                    instanceId, timeBucket, agg5m.getBackendType());
        }
    }
